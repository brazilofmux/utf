 ...
 */
UTF_API size_t co_visible_length(const unsigned char *p, size_t len);
 ...
 */
UTF_API const unsigned char *co_skip_color(const unsigned char *p,
 ...
 */
UTF_API const unsigned char *co_visible_advance(const unsigned char *p,
                                        const unsigned char *pe,
                                        size_t n,
                                        size_t *out_count);
 ...
 * out must have room for at least UTF_BUFSIZE bytes.
 */
UTF_API size_t co_copy_visible(unsigned char *out, const unsigned char *p,
 ...
 */
UTF_API const unsigned char *co_find_delim(const unsigned char *p,
                                   const unsigned char *pe,
                                   unsigned char delim);
 ...
 */
UTF_API size_t co_strip_color(unsigned char *out, const unsigned char *p, size_t len);
 ...
 */
UTF_API size_t co_words_count(const unsigned char *p, size_t len,
                      unsigned char delim);
 ...
 */
UTF_API size_t co_first(unsigned char *out, const unsigned char *p, size_t len,
                unsigned char delim);
 ...
 */
UTF_API size_t co_rest(unsigned char *out, const unsigned char *p, size_t len,
               unsigned char delim);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_last(unsigned char *out, const unsigned char *p, size_t len,
               unsigned char delim);
 ...
 */
UTF_API size_t co_extract(unsigned char *out,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_left(unsigned char *out,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_right(unsigned char *out,
                const unsigned char *p, size_t len, size_t n);
 ...
 */
UTF_API size_t co_trim(unsigned char *out,
               const unsigned char *p, size_t len,
 ...
 */
UTF_API const unsigned char *co_search(const unsigned char *haystack, size_t hlen,
 ...
 */
UTF_API size_t co_split_words(const unsigned char *data, size_t len,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_trim_pattern(unsigned char *out,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_compress_str(unsigned char *out,
                       const unsigned char *data, size_t len,
                       const unsigned char *sep, size_t sep_len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_toupper(unsigned char *out, const unsigned char *p, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_tolower(unsigned char *out, const unsigned char *p, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_totitle(unsigned char *out, const unsigned char *p, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_reverse(unsigned char *out, const unsigned char *p, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_edit(unsigned char *out,
...
UTF_API size_t co_transform(unsigned char *out,
                    const unsigned char *str, size_t slen,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_compress(unsigned char *out,
                   const unsigned char *p, size_t len,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_mid(unsigned char *out,
              const unsigned char *p, size_t len,
 ...
 */
UTF_API size_t co_pos(const unsigned char *haystack, size_t hlen,
              const unsigned char *needle, size_t nlen);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_lpos(unsigned char *out,
 ...
 */
UTF_API size_t co_member(const unsigned char *target, size_t tlen,
 ...
 */
UTF_API int co_console_width(const unsigned char *pCodePoint);
 ...
 */
UTF_API size_t co_visual_width(const unsigned char *p, size_t len);

/*
 * co_cluster_width — Display column width of the first grapheme cluster
 * in a PUA-colored string.
 *
 ...
 * widths, with regional-indicator pairs forced to 2 and U+FE0F (emoji
 * presentation) promoting an otherwise single-cell base to 2.
 *
 * Returns 0 if data is empty / only color.  There is no universal ground
 * truth for emoji cell width across terminals; this is a policy.
 */
UTF_API size_t co_cluster_width(const unsigned char *data, size_t len);

/*
 * co_field — (bytes written, display columns occupied).  Result of
 * co_copy_field for column-layout work.
 */
 ...
 * byte limit, cluster-safe cuts, and color close on truncate.
 *
 * Satisfies four constraints that no earlier primitive held together:
 ...
 */
UTF_API co_field co_copy_field(unsigned char *out, size_t nOutMax,
 ...
 *
 * Convenience wrapper around co_copy_field with nOutMax = UTF_BUFSIZE.
 * Returns bytes written to out (excluding NUL).
 */
UTF_API size_t co_copy_columns(unsigned char *out, const unsigned char *p,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_center(unsigned char *out,
                 const unsigned char *p, size_t len,
                 size_t width,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_ljust(unsigned char *out,
                const unsigned char *p, size_t len,
                size_t width,
                const unsigned char *fill, size_t fill_len,
                int bTrunc);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_rjust(unsigned char *out,
                const unsigned char *p, size_t len,
                size_t width,
                const unsigned char *fill, size_t fill_len,
                int bTrunc);
 ...
 *
 * Returns bytes written to out, or 0 if result would exceed UTF_BUFSIZE.
 */
UTF_API size_t co_repeat(unsigned char *out,
                 const unsigned char *p, size_t len,
                 size_t count);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_delete(unsigned char *out,
                 const unsigned char *p, size_t len,
                 size_t iStart, size_t nCount);
 ...
 */
UTF_API size_t co_splice(unsigned char *out,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_insert_word(unsigned char *out,
                      const unsigned char *list, size_t llen,
 ...
 *
 * Positions are 1-based, negative wraps from end.  Out-of-range
 * positions are ignored.  Duplicate positions are collapsed.
 * The positions array is modified (sorted in place).
 * Returns bytes written to out.
 */
UTF_API size_t co_replace_at(unsigned char *out,
                     const unsigned char *list, size_t llen,
 ...
 *
 * Positions are 1-based, negative wraps from end (+nWords+1).
 * For empty lists, only positions 1 and -1 are valid.
 * The positions array is modified (sorted in place).
 * Returns bytes written to out.
 */
UTF_API size_t co_insert_at(unsigned char *out,
                    const unsigned char *list, size_t llen,
                    int *positions, int nPositions,
                    const unsigned char *word, size_t wlen,
                    unsigned char delim, unsigned char osep);
 ...
 *
 * Positions are 1-based, negative wraps from end (+nWords).
 * Out-of-range positions are ignored.  Duplicate positions are
 * collapsed.  The positions array is modified (sorted in place).
 * Returns bytes written to out.
 */
UTF_API size_t co_delete_at(unsigned char *out,
                    const unsigned char *list, size_t llen,
                    int *positions, int nPositions,
                    unsigned char delim, unsigned char osep);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_sort_words(unsigned char *out,
                     const unsigned char *list, size_t llen,
                     unsigned char delim, unsigned char osep,
                     char sort_type);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_setunion(unsigned char *out,
                   const unsigned char *list1, size_t len1,
                   const unsigned char *list2, size_t len2,
                   unsigned char delim, unsigned char osep,
                   char sort_type);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_setdiff(unsigned char *out,
                  const unsigned char *list1, size_t len1,
                  const unsigned char *list2, size_t len2,
                  unsigned char delim, unsigned char osep,
                  char sort_type);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_setinter(unsigned char *out,
                   const unsigned char *list1, size_t len1,
                   const unsigned char *list2, size_t len2,
                   unsigned char delim, unsigned char osep,
                   char sort_type);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_collapse_color(unsigned char *out,
                         const unsigned char *p, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_apply_color(unsigned char *out,
                      const unsigned char *p, size_t len,
                      co_ColorState cs);
 ...
 */
UTF_API size_t co_merge(unsigned char *out,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_escape(unsigned char *out,
                 const unsigned char *data, size_t len);
 ...
 */
UTF_API size_t co_cluster_count(const unsigned char *data, size_t len);
 ...
 */
UTF_API const unsigned char *co_cluster_advance(const unsigned char *data,
                                        const unsigned char *pe,
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_mid_cluster(unsigned char *out,
                      const unsigned char *data, size_t len,
                      size_t iStart, size_t nCount);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_delete_cluster(unsigned char *out,
                         const unsigned char *data, size_t len,
                         size_t iStart, size_t nCount);
 ...
 */
UTF_API unsigned char co_dfa_ascii(const unsigned char *p);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_render_ascii(unsigned char *out,
                       const unsigned char *data, size_t len);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_render_ansi16(unsigned char *out,
                        const unsigned char *data, size_t len,
                        int bNoBleed);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_render_ansi256(unsigned char *out,
                         const unsigned char *data, size_t len,
                         int bNoBleed);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_render_truecolor(unsigned char *out,
                           const unsigned char *data, size_t len,
                           int bNoBleed);
 ...
 * Returns bytes written to out.
 */
UTF_API size_t co_render_html(unsigned char *out,
                      const unsigned char *data, size_t len);
 ...
 *
 * out_attrs:  Must have room for at least UTF_BUFSIZE entries.
 * out_text:   Must have room for at least UTF_BUFSIZE bytes.
 ...
 */
UTF_API size_t co_render_attrs(co_color_attr *out_attrs,
 ...
 */
UTF_API size_t co_parse_ansi(const unsigned char *src, size_t src_len,
