 ...
 *   - Deterministic edge cases (empty, single char, boundary, UTF_BUFSIZE)
...
static size_t gen_random_colored(unsigned char *buf, size_t max_visible) {
    size_t pos = 0;
    size_t vis = 0;
    while (vis < max_visible && pos + 20 < UTF_BUFSIZE) {
...
static void test_visible_length(void) {
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        unsigned char stripped[UTF_BUFSIZE];
...
static void test_strip_color(void) {
    ...
    unsigned char buf[UTF_BUFSIZE], out[UTF_BUFSIZE];
...
static void test_first_rest_last(void) {
    unsigned char out[UTF_BUFSIZE], buf[UTF_BUFSIZE];
    size_t r;
    ...
    unsigned char expect_first[UTF_BUFSIZE];
    size_t en = 0;
...
static void test_case(void) {
    ...
    unsigned char expect[UTF_BUFSIZE];
    size_t en = 0;
...
static void test_fuzz_invariants(void) {
    ...
    unsigned char buf[UTF_BUFSIZE], out1[UTF_BUFSIZE], out2[UTF_BUFSIZE];
    int failures = 0;
    ...
    for (int i = 0; i < 10000 && failures < 5; i++) {
        ...
        unsigned char sr1[UTF_BUFSIZE], sr2[UTF_BUFSIZE];
...
/* ---- Boundary: UTF_BUFSIZE edge ---- */

static void test_lbuf_boundary(void) {
    const char *name = "lbuf_boundary";
    unsigned char buf[UTF_BUFSIZE], out[UTF_BUFSIZE];

    /* Fill to exactly UTF_BUFSIZE-1 with 'A'. */
    memset(buf, 'A', UTF_BUFSIZE - 1);
    size_t len = UTF_BUFSIZE - 1;

    size_t vis = co_visible_length(buf, len);
    check_size(name, "vis_length at LBUF-1", vis, UTF_BUFSIZE - 1);

    size_t r = co_left(out, buf, len, UTF_BUFSIZE);
    check_size(name, "left(LBUF) len", r, UTF_BUFSIZE - 1);

    r = co_reverse(out, buf, len);
    check_size(name, "reverse LBUF-1", r, UTF_BUFSIZE - 1);

    /* repeat that would exceed UTF_BUFSIZE. */
    r = co_repeat(out, (const unsigned char *)"AAAA", 4, UTF_BUFSIZE);
    /* Should be truncated or return 0. */
    if (r == 0 || r <= UTF_BUFSIZE) {
        test_ok(name, "repeat overflow handled (r=%zu)", r);
    } else {
        test_fail(name, "repeat overflow: r=%zu exceeds UTF_BUFSIZE", r);
    }
}
...
static void test_color_stress(void) {
    ...
    unsigned char buf[UTF_BUFSIZE], out[UTF_BUFSIZE], out2[UTF_BUFSIZE];
    size_t n, r;
    ...
    unsigned char stripped_w2[UTF_BUFSIZE];
    ...
    for (int i = 0; i < 5000 && failures < 3; i++) {
        ...
        while (vis_emitted < target_vis && len + 20 < UTF_BUFSIZE) {
        ...
        if (v > 0) {
            ...
            unsigned char ls[UTF_BUFSIZE], rs[UTF_BUFSIZE];
     ...
     *     lands right at UTF_BUFSIZE - 1.
     */
    n = 0;
    /* Fill with FG(i) + 'A' pairs until near limit. */
    while (n + 4 < UTF_BUFSIZE - 1) {  /* 3 bytes color + 1 byte char = 4 */
...
static void test_cluster_color_stress(void) {
    ...
    for (int i = 0; i < 5000 && failures < 3; i++) {
        ...
        for (size_t j = 0; j < target && n + 10 < UTF_BUFSIZE; j++) {
            if (xrand() % 3 == 0)
...
static void test_edit_adversarial(void) {
    ...
    /* 11. Repeated edit that would expand past UTF_BUFSIZE. */
    unsigned char big[UTF_BUFSIZE];
    memset(big, 'a', 1000);
    r = co_edit(out, big, 1000,
        (const unsigned char *)"a", 1,
        (const unsigned char *)"1234567890", 10);
    /* Should produce at most UTF_BUFSIZE-1 bytes, silently truncated. */
    if (r > 0 && r < UTF_BUFSIZE) {
...
static void test_sort_adversarial(void) {
    ...
    unsigned char out2[UTF_BUFSIZE];
    ...
    for (int i = 0; i < 2000 && failures < 3; i++) {
        ...
        for (int w = 0; w < nwords && n + 10 < UTF_BUFSIZE; w++) {
            if (w > 0) buf[n++] = ' ';
...
static void test_set_adversarial(void) {
    ...
    unsigned char buf[UTF_BUFSIZE], a_union_b[UTF_BUFSIZE], a_inter_b[UTF_BUFSIZE];
    unsigned char a_minus_b[UTF_BUFSIZE], b_minus_a[UTF_BUFSIZE];
    unsigned char lhs[UTF_BUFSIZE], rhs[UTF_BUFSIZE];
    ...
    for (int ii = 0; ii < 2000 && failures < 3; ii++) {
        ...
        for (int w = 0; w < wa && na + 5 < UTF_BUFSIZE; w++) {
            if (w > 0) buf[na++] = ' ';
            buf[na++] = (unsigned char)('a' + (xrand() % 10));
        }
        unsigned char buf2[UTF_BUFSIZE];
        for (int w = 0; w < wb && nb + 5 < UTF_BUFSIZE; w++) {
            if (w > 0) buf2[nb++] = ' ';
...
static void test_lbuf_overflow(void) {
    ...
    r = co_ljust(out, (const unsigned char *)"x", 1, UTF_BUFSIZE + 100,
                 NULL, 0, 0);
    if (r > 0 && r < UTF_BUFSIZE) {
    ...
    r = co_center(out, (const unsigned char *)"X", 1, UTF_BUFSIZE - 1,
...
static void test_copy_columns(void) {
    ...
    /* Large: copy exact UTF_BUFSIZE-1 columns of ASCII. */
    unsigned char big[UTF_BUFSIZE];
    memset(big, 'X', UTF_BUFSIZE - 1);
    r = co_copy_columns(out, big, big + UTF_BUFSIZE - 1, UTF_BUFSIZE - 1);
    check_size(name, "LBUF-1 cols", r, UTF_BUFSIZE - 1);
}
